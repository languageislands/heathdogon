...
def parse_noun_data(noun):
    ...
    if noun.endswith("y") or (noun.endswith("ⁿ") and noun[-2] == "y"):
        parsed_noun = noun_parser.exception_in_penultimate_position(
        noun_parser.special_nouns(
...
def parse_verb_data(verb):
    if pd.isna(verb):
        return None
    parsed_verb =verb_parser.exception_in_penultimate_position(
        verb_parser.special_verbs(
...
def parse_adj_num_data(adjective_numeral):
    if pd.isna(adjective_numeral):
        return None
    parsed_adj_num = adj_num_parser.exception_in_penultimate_position(
        adj_num_parser.double_last_vowels(
